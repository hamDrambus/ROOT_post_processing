...
//	
	...
	int area_vector::get_next_index(void) //for running through all indices
	{
		...
		std::deque<int>::iterator right = _last_returned_index_left + 1;
	...
	Bool_t area_vector::contains(int index)
	{
	...
	std::deque<area_vector> area_vector::split_area(int N)
	{
	...
	Bool_t experiment_area::contains(/*ParameterPile::*/experiment_area what)
	{
		if ((what._type != Type::Point) || Type::Area != _type)
			return kFALSE; //not implemented intersection of two areas
		if (!isValid())
			return kFALSE;
