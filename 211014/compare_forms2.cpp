...
//For 211014/results_v5, based on 211014/compare_forms1.cpp
...
//Testing why SiPM's long component is sensitive to event selection (peak vs slope)
//Hypothesis is that pre-trigger baseline is incorrect due to cut on Npe[~24us, 160us] instead of Npe[0us, 160us]
...
pulse_shape SiPM_20_7kV_slope;
define = &SiPM_20_7kV_slope;
define->folder = std::string("211014/results_v5/Pu_46V_20.7kV_850V/") + ("forms_Pu_left/");
define->fnames = {"SiPMs_form_by_Npe.hdata"};
define->Td = "7.4";
define->device = "SiPM-matrix (slope)";
define->fast_t_center = 28.1;
define->fast_t = PAIR(25.0, 29.9);
define->S1_t_center = 0;
define->S1_t = PAIR(0, 0);
...
define->simultaneous_fit = false;
  ...
  pulse_shape SiPM_20_7kV_slope_v2;
  define = &SiPM_20_7kV_slope_v2;
  copy = &SiPM_20_7kV_slope;
*define = *copy;
define->slow_fit_t = PAIR(33.0, 43.0);
define->long_fit_t = PAIR(80, 154);
define->baseline_bound = PAIR(1e-6, 1.5e-3);
...
define->simultaneous_fit = false;
  ...
  std::vector<pulse_shape> pulses = {SiPM_20_7kV_no_trigger, SiPM_20_7kV_slope};
    ...
    pulses[hh].hist->Sumw2();
		...
		if (pulses[hh].renormalize) {
          ...
          if (normalize_by_noise)
            integral = integrate(pulses[hh].hist, time_pretrigger_left + pulses[hh].t_offset, time_pretrigger_right + pulses[hh].t_offset);
          else
