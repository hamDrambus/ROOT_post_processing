...
data_output_path = "190228/results_v5/";
std::map<std::string, std::pair<double, double> > S2_times;
S2_times["190228_Cd_20kV_850V_46V_th500mV"] = std::pair<double, double> (22, 40);
S2_times["190228_Cd_20kV_850V_46V_th600mV"] = std::pair<double, double> (22, 40);
S2_times["190228_Cd_18kV_850V_46V_th500mV"] = std::pair<double, double> (22, 40);
S2_times["190228_Cd_16kV_850V_46V_th500mV"] = std::pair<double, double> (22, 40);
S2_times["190228_Bkg_20kV_850V_46V_th500mV"] = std::pair<double, double> (22, 40);
...
if ((exp == "190228_Bkg_20kV_850V_46V_th500mV" || exp == "190228_Cd_20kV_850V_46V_th500mV" || exp == "190228_Cd_20kV_850V_46V_th600mV") && method == 0) {
...
set_corr(AStates::PMT_Npe_sum, AStates::PMT_Npe_sum, -1, -1);
	...
	set_bins(300);
	cut_x_y_right(22, 0, 300, 184, true, "1");
...
set_corr(AStates::PMT_trigger_bNpeaks, AStates::PMT_trigger_bNpe, -1, -1);
	...
	set_bins(600);
	cut_x_y_right(33.57, 0, 33.57, 160, true, "1");
	cut_x_y_upper(0, 34.06, 160, 34.06, true, "2");
	cut_x_y_left(30.35, 0, 30.35, 160, true, "3");
	cut_x_y_lower(0, 30.61, 160, 30.61, true, "4");
	cut_x_y_right(28.16, 28.82, 36.31, 34.66, true, "5");
	cut_x_y_left(28.11, 29.61, 34.92, 35.25, true, "6");
...
if (exp == "190228_Cd_18kV_850V_46V_th500mV" && method == 0) {
	double dt_shaping = 0.81 + 0.8228*drift_time_from_kV(experiment_fields[exp], 1.8);
	std::string dt = "dt="+dbl_to_str(dt_shaping, 1)+"us";
	std::string DT = "dt="+dbl_to_str(dt_shaping, 1)+"#mus"; //ROOT supports greek letters
	std::string cuts = "no_cuts";
...
set_corr(AStates::PMT_Npe_sum, AStates::PMT_Npe_sum, -1, -1);
	...
	set_bins(250);
	cut_x_y_right(18.5, 0, 250, 162, true, "1");
	set_titles("N_{pe} t#in["+S2_start+", 160] #mus", "N_{pe} t#in["+S2_start+", "+S2_finish+"] #mus");
	update();
	saveaspng(FOLDER + Num + "_slowPMTs_Npe_"+cuts+"_"+S2_start+"-"+S2_finish+"us_vs_"+S2_start+"-160us");
	set_as_run_cut("good_sPMTs_ZxZy");	cuts = "cuts_"+Num;
...
set_corr(AStates::PMT_trigger_bNpeaks, AStates::PMT_trigger_bNpe, -1, -1);
	...
	cut_x_y_lower(0, 30.61, 160, 30.61, true, "4");
	cut_x_y_right(28.16, 28.82, 36.31, 34.66, true, "5");
	cut_x_y_left(28.16, 29.35, 34.60, 35.32, true, "6");
	set_titles("t by fastPMTs by N " + DT, "t by slowPMTs by Npe" + DT);
	update();
	set_as_run_cut("good_trigger"); cuts += "+" + Num;
	saveaspng(FOLDER + Num+"_fastPMTs_N_vs_slowPMTs_trigger_"+cuts+"_"+dt+"_zoom");
...
if (exp == "190228_Cd_16kV_850V_46V_th500mV" && method == 0) {
	double dt_shaping = 0.81 + 0.8228*drift_time_from_kV(experiment_fields[exp], 1.8);
	std::string dt = "dt="+dbl_to_str(dt_shaping, 1)+"us";
	std::string DT = "dt="+dbl_to_str(dt_shaping, 1)+"#mus"; //ROOT supports greek letters
	std::string cuts = "no_cuts";
...
set_corr(AStates::PMT_Npe_sum, AStates::PMT_Npe_sum, -1, -1);
	...
	set_bins(200);
	cut_x_y_right(13, 0, 200, 148.5, true, "1");
	set_titles("N_{pe} t#in["+S2_start+", 160] #mus", "N_{pe} t#in["+S2_start+", "+S2_finish+"] #mus");
	update();
	saveaspng(FOLDER + Num + "_slowPMTs_Npe_"+cuts+"_"+S2_start+"-"+S2_finish+"us_vs_"+S2_start+"-160us");
	set_as_run_cut("good_sPMTs_ZxZy");	cuts = "cuts_"+Num;
...
set_corr(AStates::PMT_trigger_bNpeaks, AStates::PMT_trigger_bNpe, -1, -1);
	...
	cut_x_y_right(33.57, 0, 33.57, 160, true, "1");
	cut_x_y_upper(0, 34.06, 160, 34.06, true, "2");
	cut_x_y_left(30.22, 0, 30.22, 160, true, "3");
	cut_x_y_lower(0, 30.28, 160, 30.28, true, "4");
	cut_x_y_right(28.34, 28.75, 36.31, 34.66, true, "5");
	cut_x_y_left(27.75, 29.08, 34.60, 35.32, true, "6");
	set_titles("t by fastPMTs by N " + DT, "t by slowPMTs by Npe" + DT);
	update();
	set_as_run_cut("good_trigger"); cuts += "+" + Num;
	saveaspng(FOLDER + Num+"_fastPMTs_N_vs_slowPMTs_trigger_"+cuts+"_"+dt+"_zoom");
