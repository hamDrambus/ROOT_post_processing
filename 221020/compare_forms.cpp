...
int compare_forms (void) {
	...
	pulse_shape SiPM_18_5V_no_trigger;
	define = &SiPM_18_5V_no_trigger;
define->folder = "221020/results_v1/Pu_18.5kV_46V_850V/forms_Pu_peak/";
define->fnames = {"SiPMs_form_by_Npe.hdata"};
define->Td = "86";
define->device = "SiPM matrix";
define->fast_t_center = 32.7;
define->fast_t = PAIR(20, 50);
...
define->fit_option = def_fit_option;
	...
	pulse_shape PMT_18_5V_no_trigger;
	define = &PMT_18_5V_no_trigger;
	copy = &SiPM_18_5V_no_trigger;
*define = *copy;
define->folder = "221020/results_v1/Pu_18.5kV_46V_850V/forms_Pu_peak/";
define->fnames.clear();
if(PMT_used&0x1) define->fnames.push_back("5_form_by_Npe.hdata");
if(PMT_used&0x2) define->fnames.push_back("6_form_by_Npe.hdata");
if(PMT_used&0x4) define->fnames.push_back("7_form_by_Npe.hdata");
if(PMT_used&0x8) define->fnames.push_back("8_form_by_Npe.hdata");
define->Td = "86";
define->device = "4PMT+WLS";
define->scale = 1;


	//for paper:
	//std::vector<pulse_shape> pulses = {SiPM_18_5V_no_trigger, PMT_18_5V_no_trigger};
	//std::vector<pulse_shape> pulses = {PMT_18_5V_no_trigger};
	std::vector<pulse_shape> pulses = {SiPM_18_5V_no_trigger};

	std::vector<Color_t> palette_major = {kBlack, kRed, kBlue, kGreen, kYellow + 2, kMagenta, kOrange + 7};
	//std::vector<Color_t> palette_major = {kBlue, kRed, kBlack, kGreen, kYellow + 2, kMagenta, kOrange + 7};
	...
	if (PMTs)
		framename = std::string("Results for 4PMT (no WLS), ")+"5.5 MeV #alpha paricle";// + " " + Tds[0] + " Td";
	else
		framename = std::string("Results for SiPM-matrix (no WLS), ")+"5.5 MeV #alpha paricle";// + " " + Tds[0] + " Td";
  ...
  max_val*= linear ? 1.2 : 3;
	...
	//=====================================
	if (!linear)
		frame->GetXaxis()->SetRangeUser(40, 120);
	else
		frame->GetXaxis()->SetRangeUser(80, 100);
	//=====================================
	...
	for (int hh = 0, hh_end_ = pulses.size(); hh!=hh_end_; ++hh) {
		legend->AddEntry(pulses[hh].hist, (std::string("E_{THGEM} = ") + pulses[hh].Td + " kV/cm, " + pulses[hh].device).c_str(), "l");
		//legend->AddEntry(pulses[hh].hist, (std::string("E_{GEM} = ") + pulses[hh].Td + " kV/cm").c_str(), "l");
	}
