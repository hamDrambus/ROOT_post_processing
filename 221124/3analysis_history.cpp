...
void noise_cut(int channel, int aggressiveness, int device_condition, bool display) {
  ...
  if (1 == device_condition) {
    if (channel==1) {
      cut_A_S_upper(0.005, 0.0, 0.0153, 0.0, display, channel, "rem_smallA_largeS_1");
      cut_A_S_lower(0.013, 0.0018, 0.06, 0.0069, display, channel, "rem_smallA_smallS");
      cut_A_S_lower(0.06, 0.0069, 1.4, 0.31, display, channel, "rem_largeA_smallS");
    }
    if (channel==2) {
      cut_A_S_upper(0.005, 0.0, 0.014, 0.0, display, channel, "rem_smallA_largeS_1");
      cut_A_S_lower(0.010, 0.00172, 0.055, 0.0044, display, channel, "rem_smallA_smallS");
      cut_A_S_lower(0.055, 0.0044, 1.6, 0.27, display, channel, "rem_largeA_smallS");
    }
    if (channel==3) {
      cut_A_S_upper(0.005, 0.0, 0.015, 0.0, display, channel, "rem_smallA_largeS");
      cut_A_S_lower(0.01, 0.0017, 0.050, 0.0040, display, channel, "rem_smallA_smallS");
      cut_A_S_lower(0.050, 0.0040, 1.2, 0.22, display, channel, "rem_largeA_smallS");
    }
    if (channel==4) {
      cut_A_S_upper(0.01, 0.0, 0.0142, 0.0, display, channel, "rem_smallA_largeS_1");
      cut_A_S_lower(0.01, 0.0015, 0.055, 0.0044, display, channel, "rem_smallA_smallS");
      cut_A_S_lower(0.055, 0.0044, 1.6, 0.26, display, channel, "rem_largeA_smallS");
    }
    if (channel==5) {
      cut_A_S_upper(0.001, 0.0, 0.0055, 0.0, display, channel, "rem_smallA-S");
      if (aggressiveness == 1) {
        cut_A_S_upper(0.65, 0, 5, 0, display, channel, "rem_A>0.65");
        cut_A_S_upper(0.0, 0.030, 1.6, 0.030, display, channel, "rem_S>0.030");
      }
    ...
    if (channel==6) {
      cut_A_S_upper(0.001, 0.0, 0.0057, 0.0, display, channel, "rem_smallA-S");
      if (aggressiveness == 1) {
        cut_A_S_upper(0.50, 0, 5, 0, display, channel, "rem_A>0.50");
        cut_A_S_upper(0.0, 0.020, 1.6, 0.020, display, channel, "rem_S>0.020");
      }
    ...
    if (channel==7) {
      cut_A_S_upper(0.001, 0.0, 0.0058, 0.0, display, channel, "rem_smallA-S");
      if (aggressiveness == 1) {
        cut_A_S_upper(0.54, 0, 2, 0, display, channel, "rem_A>0.54");
        cut_A_S_upper(0.0, 0.020, 1.6, 0.020, display, channel, "rem_S>0.020");
      }
...
// Output is signal pulse-shapes
// Same as 221124/1analysis_history.cpp, but for data with grounded GEM1
...
data_output_path = "221124/results_v3/";
calibration_file = "221124/results_v3/221124_calibration.dat";
...
for (int e = 0; e != exp_area.experiments.size(); ++e) {
  ...
  S2_times[exp] = std::pair<double, double> (43, 158);
...
exp_folders["221124_X-ray_Q_20kV_850V_46V_14mm_coll_filt3"] = "X-ray_14mm_filt3_20kV";
exp_folders["221124_X-ray_Q_18kV_850V_46V_14mm_coll_filt3"] = "X-ray_14mm_filt3_18kV";
exp_folders["221124_X-ray_Q_16kV_850V_46V_14mm_coll_filt3"] = "X-ray_14mm_filt3_16kV";
exp_folders["221124_X-ray_Q_14kV_850V_46V_14mm_coll_filt3"] = "X-ray_14mm_filt3_14kV";
exp_folders["221124_X-ray_Q_12kV_850V_46V_14mm_coll_filt3"] = "X-ray_14mm_filt3_12kV";
exp_folders["221124_X-ray_Q_101kV_850V_46V_14mm_coll_filt3"] = "X-ray_14mm_filt3_10kV";
exp_folders["221124_X-ray_Q_8kV_800V_46V_14mm_coll_filt3"] = "X-ray_14mm_filt3_08kV";
...
for (std::size_t exp_ind = 0; calibrate && (exp_ind!= exp_area.experiments.size()); ++exp_ind) {
  ...
  int PMT_state = PMT_V[exp] == 850 ? 0 : 1; //0 for 850V, 12 dB, 1 for 800V
...
//zcxv
if (exp == "221124_X-ray_Q_20kV_850V_46V_14mm_coll_filt3"
    || exp =="221124_X-ray_Q_18kV_850V_46V_14mm_coll_filt3"
    || exp =="221124_X-ray_Q_16kV_850V_46V_14mm_coll_filt3"
    || exp =="221124_X-ray_Q_14kV_850V_46V_14mm_coll_filt3"
    || exp =="221124_X-ray_Q_12kV_850V_46V_14mm_coll_filt3"
    || exp =="221124_X-ray_Q_10kV_850V_46V_14mm_coll_filt3"
    || exp =="221124_X-ray_Q_8kV_800V_46V_14mm_coll_filt3") {
...
set_corr(AStates::PMT_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
  ...
  set_bins(200);
  x_y_regions = {0, 0, 25, 0, 25, 18, 1.3, 18, 0, 4.1};
...
ty(AStates::PMT_Npe_sum);
  ...
  for (int ich =0; ich!= post_processor->MPPC_channels.size(); ++ich) {
    ...
    update();
    (*gSiPM_Npe_data.info(chan))[post_processor->current_exp_index].t_pre_trigger = std::make_pair(d_Bkg_start, d_Bkg_finish);
    ...
    update();
    (*gSiPM_Npe_data.info(chan))[post_processor->current_exp_index].t_S1 = std::make_pair(d_S1_start, d_S1_finish);
    (*gSiPM_Npe_data.info(chan))[post_processor->current_exp_index].Npe_S1 = get_mean();
    cut_t(d_S2_start, d_S2_finish, false, chan);
    update();
    (*gSiPM_Npe_data.info(chan))[post_processor->current_exp_index].t_S2 = std::make_pair(d_S2_start, d_S2_finish);
