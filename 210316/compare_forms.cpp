...
int compare_forms (std::string in_kV, std::string in_is_linear, std::string in_is_orthogonal,
...
int compare_forms (void) {
	...
	bool combined = true;
	bool orthogonal_tracks = false;
	int Nbins = 100;
	bool linear = 0;
	bool PMTs = true;
#ifdef FAST_FIGURES_MODE
	linear = (in_is_linear == "lin");
	orthogonal_tracks = (in_is_orthogonal == "orth");
	Nbins = in_Nbins;
	...
	double time_pretrigger_left = 4.3, time_pretrigger_right = 14.2;
...
define->folder = std::string("210128/results_v5/Pu_46V_16kV_850V/") + (Cd_peak ? "forms_Alpha_peak/" : "forms_Alpha_left/");
...
define->folder = std::string("210302/results_v2/Pu_46V_7.6kV_800V_131K/") + (orthogonal_tracks ? "forms_Alpha_peak_v1/" : "forms_Alpha_peak_v2/");
define->fnames = {"SiPMs_form_by_Npe.hdata"};
define->Td = "6.8";
define->device = "SiPM-matrix, 131K";
define->fast_t_center = 28.8;
define->fast_t = PAIR(25, 55.2);
define->S1_t_center = 0;
define->S1_t = PAIR(0, 0);
define->scale = 1;
define->subtract_baseline = subtact_baseline;
define->renormalize = true;
define->slow_fit_t = PAIR(56.4, 147);
define->long_fit_t = PAIR(56.4, 147);
...
define->folder = std::string("210311/results_v1/Pu_50V_7.6kV_800V_200K/") + (orthogonal_tracks ? "forms_Alpha_peak_v1/" : "forms_Alpha_peak_v2/");
define->fnames = {"SiPMs_form_by_Npe.hdata"};
define->Td = "6.8";
define->device = "SiPM-matrix, 200K";
define->fast_t_center = 28.8;
define->fast_t = PAIR(24, 55.2);
define->S1_t_center = 0;
define->S1_t = PAIR(0, 0);
define->scale = 1;
define->subtract_baseline = subtact_baseline;
define->renormalize = true;
define->slow_fit_t = PAIR(56.4, 150);
define->long_fit_t = PAIR(56.4, 150);
define->baseline_bound = PAIR(1e-6, 1e-5);
define->slow_ampl_bound = PAIR(1e-3, 1);
define->slow_tau_bound = PAIR(2.5, 10);
...
define->folder = std::string("210316/results_v1/Pu_54V_7.6kV_800V_256K/") + (orthogonal_tracks ? "forms_Alpha_peak_v1/" : "forms_Alpha_peak_v2/");
define->fnames = {"SiPMs_form_by_Npe.hdata"};
define->Td = "6.8";
define->device = "SiPM-matrix, 256K";
define->fast_t_center = 28.8;
define->fast_t = PAIR(25.5, 56.0);
define->S1_t_center = 0;
define->S1_t = PAIR(0, 0);
define->scale = 1;
define->subtract_baseline = subtact_baseline;
define->renormalize = true;
define->slow_fit_t = PAIR(57.3, 140);
define->long_fit_t = PAIR(57.3, 140);
define->baseline_bound = PAIR(1e-6, 1e-5);
define->slow_ampl_bound = PAIR(1e-3, 1);
define->slow_tau_bound = PAIR(2.5, 10);
define->long_ampl_bound = PAIR(1e-4, 1e-3);
...
define->fast_t = PAIR(25, 56);
define->S1_t_center = 0;
define->S1_t = PAIR(0, 0);
define->scale = 1;
define->subtract_baseline = subtact_baseline;
define->renormalize = true;
define->slow_fit_t = PAIR(56, 147);
define->long_fit_t = PAIR(56, 147);
define->baseline_bound = PAIR(1e-7, 3e-6);
define->slow_ampl_bound = PAIR(1e-3, 1);
define->slow_tau_bound = PAIR(2.5, 8);
define->long_ampl_bound = PAIR(2e-5, 2e-2);
define->long_tau_bound = PAIR(8, 200);
define->simultaneous_fit = true;
define->do_fit = do_fit;
define->fit_option = def_fit_option;
...
define->device += ", 256K";
define->fast_t_center = 28.7;
define->fast_t = PAIR(25, 56.4);
define->S1_t_center = 0;
define->S1_t = PAIR(0, 0);
define->scale = 1;
define->subtract_baseline = subtact_baseline;
define->renormalize = true;
define->slow_fit_t = PAIR(58.0, 135);
define->long_fit_t = PAIR(58, 135);
define->baseline_bound = PAIR(1e-7, 6e-6);
define->slow_ampl_bound = PAIR(1e-3, 1);
define->slow_tau_bound = PAIR(2.5, 8);
define->long_ampl_bound = PAIR(2e-5, 2e-2);
define->long_tau_bound = PAIR(8, 200);
...
define->fit_option = def_fit_option;
	...
	std::string folder = PMTs ? std::string("210311/results_v1/PMTs/")
					: std::string("210311/results_v1/SiPMs/");
#ifdef FAST_FIGURES_MODE
	...
	std::vector<pulse_shape> pulses = {SiPM_7_6kV_131K_no_trigger, SiPM_7_6kV_200K_no_trigger, SiPM_7_6kV_256K_no_trigger, SiPM_16kV_87K_no_trigger};
	//std::vector<pulse_shape> pulses = {PMT4_7_6kV_131K_no_trigger, PMT4_7_6kV_200K_no_trigger, PMT4_7_6kV_256K_no_trigger};
	...
	if (PMTs)
		framename = std::string("Results for 4PMT (no WLS), ")+"5.6 MeV #alpha ^{238}Pu "+(orthogonal_tracks ? "orth. tracks" : "long. tracks");
	else
		framename = std::string("Results for SiPM-matrix (no WLS), ")+"5.6 MeV #alpha ^{238}Pu "+(orthogonal_tracks ? "orth. tracks" : "long. tracks");
	...
	if (do_fit) {
		if (!linear) {
			...
			if (print_errors) {
				...
				add_text(80, 0.0066, no_title, tau1, palette_major);
				add_text(102, 0.0066, Slow_title, frsS, palette_major);
				add_text(117, 0.0066, Long_title, frsL, palette_major);
				add_text(135, 0.0066, no_title, tau2, palette_major);
			}
		} else {
			std::vector<std::string> no_title;
