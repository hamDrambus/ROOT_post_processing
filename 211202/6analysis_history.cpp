...
//Studing these distributions for fast only, slow+long only and slow component only photoelectons
data_output_path = "211202/results_v6/";
...
set_corr(AStates::PMT_Npe_sum, AStates::PMT_Npe_sum, -1, -1);
	...
	set_bins(250);
	x_y_regions = {11.3, 0, 92, 3.6, 249, 100};
...
set_corr(AStates::MPPC_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	set_bins(120);
	x_y_regions = {0.6, 0, 8.7, 2.3, 45.1, 3.7, 119, 65.0};
	cut_x_y_right(x_y_regions, true, "1");
	update();
	set_titles("N_{pe} t#in[0, 160] #mus", "N_{pe} t#in["+S2_start+", "+S2_finish+"] #mus");
	...
	double slow_cutoff = 50;//us
	std::string s_slow_cutoff = dbl_to_str(slow_cutoff, 0);
ty(AStates::PMT_Npe_sum);
	time_zoom_sPMTs(d_S2_start, 160);	set_bins(0, 200);
	draw_limits(62, 106.5);
	saveaspng(FOLDER + Num + "_sPMTs_Npe_"+cuts_str(cuts)+"_"+S2_start+"-160us_N="+int_to_str(post_processor->numOfFills(true))+"events");
	...
	saveaspng(FOLDER + Num + "_sPMTs_Npe_"+cuts_str(cuts)+"_"+S2_start+"-"+S2_finish+"us");
	Num = int_to_str(++no, 2);

	time_zoom_sPMTs(d_S2_finish, slow_cutoff); update(); //Slow component only
	saveaspng(FOLDER + Num + "_sPMTs_Npe_"+cuts_str(cuts)+"_"+S2_finish+"-"+s_slow_cutoff+"us");
	Num = int_to_str(++no, 2);

	time_zoom_sPMTs(d_S2_finish, 160); update();
	saveaspng(FOLDER + Num + "_sPMTs_Npe_"+cuts_str(cuts)+"_"+S2_finish+"-160us");
	Num = int_to_str(++no, 2);

ty(AStates::MPPC_Npe_sum);
	time_zoom_SiPMs(d_S2_start, 160);	set_bins(0, 200);
...
ty(AStates::MPPC_coord);
	...
	set_zoom(-15, 15, -15, 15);	set_bins(79);
