...
//Studing xy coordinate, coordinate 2D dispersion and Npe profiles for SiPM-matrix for alpha events
//Studing these distributions for fast only, slow only and slow+long photoelectons

data_output_path = "211028/results_v6/";
...
S2_times["211028_Pu_7.8kV_850V_46V_12dB_2650V"] = std::pair<double, double> (24, 31.8);
...
S2_times["211028_Pu_7.8kV_850V_46V_12dB_2083V"] = std::pair<double, double> (24.5, 32.0);
...
set_corr(AStates::MPPC_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	set_zoom(0, 1200, 0, 1200);	set_bins(600);
	x_y_regions = {140, 0, 274, 44.5, 666, 120, 1199, 504};
	...
	double slow_cutoff = 48;//us
	std::string s_slow_cutoff = dbl_to_str(slow_cutoff, 0);
ty(AStates::MPPC_Npe_sum);//hkjl
	time_zoom_SiPMs(d_S2_start, 160);
	set_zoom(0, 1000); set_bins(500);
	draw_limits(501, 686);
	...
	saveaspng(FOLDER + Num + "_SiPMs_Npe_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	Num = int_to_str(++no, 2);
...
ty(AStates::MPPC_coord);
	cut_SiPM_noise(SiPM_state);
	time_zoom_SiPMs(d_S2_start, 160);
	set_zoom(-12, 12, -12, 12);	set_bins(89);
	saveaspng(FOLDER + Num + "_SiPMs_xy_"+cuts_str(cuts)+"_"+S2_start+"-160us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(0, d_S2_start); update();
	saveaspng(FOLDER + Num + "_SiPMs_xy_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	Num = int_to_str(++no, 2);
...
ty(AStates::MPPC_coord_disp);
	cut_SiPM_noise(SiPM_state);
	time_zoom_SiPMs(d_S2_start, 160);
	set_zoom(0, 400); set_bins(400);
	saveaspng(FOLDER + Num + "_SiPMs_xy_disp_"+cuts_str(cuts)+"_"+S2_start+"-160us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(0, d_S2_start); update();
	saveaspng(FOLDER + Num + "_SiPMs_xy_disp_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_start, d_S2_finish); update();
	saveaspng(FOLDER + Num + "_SiPMs_xy_disp_"+cuts_str(cuts)+"_"+S2_start+"-"+S2_finish+"us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_finish, slow_cutoff); update();
	saveaspng(FOLDER + Num + "_SiPMs_xy_disp_"+cuts_str(cuts)+"_"+S2_finish+"-"+s_slow_cutoff+"us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_finish, 160); update();
	saveaspng(FOLDER + Num + "_SiPMs_xy_disp_"+cuts_str(cuts)+"_"+S2_finish+"-160us");
	Num = int_to_str(++no, 2);

ty(AStates::MPPC_coord_x);
	cut_SiPM_noise(SiPM_state);
	time_zoom_SiPMs(d_S2_start, 160);
	set_zoom(-12, 12); set_bins(99);
	saveaspng(FOLDER + Num + "_SiPMs_x_"+cuts_str(cuts)+"_"+S2_start+"-160us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(0, d_S2_start); update();
	saveaspng(FOLDER + Num + "_SiPMs_x_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	Num = int_to_str(++no, 2);
...
ty(AStates::MPPC_coord_y);
	...
	time_zoom_SiPMs(0, d_S2_start); update();
	saveaspng(FOLDER + Num + "_SiPMs_y_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	Num = int_to_str(++no, 2);
...
ty(AStates::MPPC_Npe_profile);
	...
	time_zoom_SiPMs(0, d_S2_start); update();
	saveaspng(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	Num = int_to_str(++no, 2);
...
ty(AStates::MPPC_Npe_profile_x);
	...
	time_zoom_SiPMs(0, d_S2_start); update();
	saveaspng(FOLDER + Num + "_SiPMs_profile_x_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	Num = int_to_str(++no, 2);
...
ty(AStates::MPPC_Npe_profile_y);
	...
	time_zoom_SiPMs(0, d_S2_start); update();
	saveaspng(FOLDER + Num + "_SiPMs_profile_y_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	Num = int_to_str(++no, 2);
...
ty(AStates::MPPC_coord);
	time_zoom_SiPMs(d_S2_start, d_S2_finish);
	x_y_regions = {-0.90*1.2, -1.80*1.2, 0.90*1.2, -1.80*1.2, 1.60*1.2, 0*1.2, 0.90*1.2, 1.80*1.2, -0.90*1.2, 1.80*1.2, -1.60*1.2, 0.0*1.2};
...
set_corr(AStates::MPPC_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	time_zoom_SiPMs(d_S2_start, d_S2_finish);
	ty(AStates::Correlation);
	set_zoom(0, 600, 0, 600);	set_bins(600);
	x_y_regions = {28, 0, 77, 27, 198, 56, 599, 381};
	cut_x_y_right(x_y_regions, true, "1");
	update();
	set_titles("N_{pe} t#in[0, 160] #mus", "N_{pe} t#in["+S2_start+", "+S2_finish+"] #mus");
	saveaspng(FOLDER + Num + "_SiPMs_Npe_"+cuts_str(cuts)+"_"+S2_start+"-"+S2_finish+"us_vs_0-160us");
