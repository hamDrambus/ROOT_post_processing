...
int compare_forms (void) {
	...
	double time_pretrigger_left = 3, time_pretrigger_right = 20;
...
define->folder = std::string("210302/results_v1/Pu_46V_7.6kV_800V_120K/") + (orthogonal_tracks ? "forms_Alpha_peak_v1/" : "forms_Alpha_peak_v2/");
define->fnames = {"SiPMs_form_by_Npe.hdata"};
define->Td = "6.8";
define->device = "SiPM-matrix, 120K";
define->fast_t_center = 28.8;
define->fast_t = PAIR(40, 72);
define->S1_t_center = 0;
define->S1_t = PAIR(0, 0);
define->scale = 1;
define->subtract_baseline = subtact_baseline;
define->renormalize = true;
define->slow_fit_t = PAIR(73.5, 148);
define->long_fit_t = PAIR(73.5, 148);
...
define->folder = std::string("210302/results_v1/Pu_46V_7.6kV_800V_131K/") + (orthogonal_tracks ? "forms_Alpha_peak_v1/" : "forms_Alpha_peak_v2/");
define->fnames = {"SiPMs_form_by_Npe.hdata"};
define->Td = "6.8";
define->device = "SiPM-matrix, 131K";
define->fast_t_center = 28.8;
define->fast_t = PAIR(25, 52);
define->S1_t_center = 0;
define->S1_t = PAIR(0, 0);
define->scale = 1;
define->subtract_baseline = subtact_baseline;
define->renormalize = true;
define->slow_fit_t = PAIR(56.0, 147);
define->long_fit_t = PAIR(56.0, 147);
...
define->fast_t = PAIR(40, 69);
define->S1_t_center = 0;
define->S1_t = PAIR(0, 0);
define->scale = 1;
define->subtract_baseline = subtact_baseline;
define->renormalize = true;
define->slow_fit_t = PAIR(78, 152);
define->long_fit_t = PAIR(78, 152);
define->baseline_bound = PAIR(1e-6, 2e-4);
define->slow_ampl_bound = PAIR(1e-3, 1);
define->slow_tau_bound = PAIR(2.5, 10);
...
define->subtract_baseline = subtact_baseline;
define->renormalize = true;
define->slow_fit_t = PAIR(56, 100);
define->long_fit_t = PAIR(56, 100);
define->baseline_bound = PAIR(1e-6, 2e-4);
define->slow_ampl_bound = PAIR(1e-3, 1);
...
define->fit_option = def_fit_option;
	...
	std::string folder = PMTs ? std::string("210302/results_v1/PMTs/")
					: std::string("210302/results_v1/SiPMs/");
#ifdef FAST_FIGURES_MODE
	std::vector<pulse_shape> pulses;
	if (PMTs) {
		if (in_kV == "7.6")
			pulses.push_back((combined ? PMT4_7_6kV_120K_no_trigger :  PMT4_7_6kV_120K_no_trigger_v2));
	} else {
		if (in_kV == "7.6")
			pulses.push_back((combined ? SiPM_7_6kV_120K_no_trigger :  SiPM_7_6kV_120K_no_trigger_v2));
	}
#else //FAST_FIGURES_MODE

	std::vector<pulse_shape> pulses = {SiPM_7_6kV_120K_no_trigger};
	//std::vector<pulse_shape> pulses = {PMT4_7_6kV_120K_no_trigger};
	//std::vector<pulse_shape> pulses = {SiPM_7_6kV_131K_no_trigger};
	//std::vector<pulse_shape> pulses = {PMT4_7_6kV_131K_no_trigger};
  ...
  max_val*= linear ? 1.2 : 2;
	...
	if (!linear) {
		...
		if (print_errors) {
			...
			add_text(80, 0.026, no_title, tau1, palette_major);
			add_text(102, 0.026, Slow_title, frsS, palette_major);
			add_text(117, 0.026, Long_title, frsL, palette_major);
			add_text(135, 0.026, no_title, tau2, palette_major);
		}
	} else {
		std::vector<std::string> no_title;
