...
//As 210316/2analysis_history.cpp: usual + trigger cut but no adjustment and no track selection
data_output_path = "210302/results_v3/";
...
set_corr(AStates::PMT_Npe_sum, AStates::PMT_Npe_sum, -1, -1);
	...
	set_zoom(55, 90); set_bins(1000);
	draw_limits(66.0, 73.5);
...
set_corr(AStates::PMT_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	x_y_regions = {585, 364, 275, 778, 431, 953, 744, 542};
...
set_corr(AStates::PMT_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	set_bins(1500);
	set_titles("N_{pe} PMTs", "N_{pe} SiPM-matrix");
	x_y_regions = {530, 255, 267, 610, 393, 739, 647, 384};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_slowPMTs_vs_SiPMs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
