...
//Second analysis of cryogenic temperature gaseous Ar alpha data
//Trying to use PMT_T_sum to select orthogonal alpha orthogonal_tracks
//and MPPC_trigger_avg to adjust/cut trigger jitter
data_output_path = "210302/results_v2/";
...
set_corr(AStates::PMT_Npe_sum, AStates::PMT_Npe_sum, -1, -1);
	...
	set_zoom(60, 95); set_bins(1000);
	draw_limits(70, 75);
	saveaspng(FOLDER + Num+"_SiPMs_trigger_avg_"+cuts_str(cuts));
	set_as_run_cut("SiPMs_trigger"); cuts.push_back(Num);
	post_processor->set_trigger_offsets(72.2);
	Num = int_to_str(++no, 2);

std::string form_n = "forms_Alpha_peak_v2/";
set_corr(AStates::PMT_Npe_sum, AStates::PMT_T_sum, -1, -1);
	ty(AStates::Correlation);
	set_zoom(0, 1000, 0, 15);
	set_bins(1000, 500);
	x_y_regions = {386, 5.15, 461, 6.95, 383, 7.24, 325, 5.93};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	set_titles("N_{pe} sPMT t#in[0, 160] #mus", "Signal width trigger, [#mus]");
	saveaspng(FOLDER + Num+"_triggerPMT_Tsum_vs_slowPMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
...
form_n = "forms_Alpha_peak_v1/";
	ty(AStates::Correlation);
	x_y_regions = {504, 5.12, 505, 6.93, 669, 7.24, 677, 5.93};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	set_titles("N_{pe} sPMT t#in[0, 160] #mus", "Signal width trigger, [#mus]");
	saveaspng(FOLDER + Num+"_triggerPMT_Tsum_vs_slowPMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
...
set_corr(AStates::PMT_Npe_sum, AStates::PMT_Npe_sum, -1, -1);
	...
	set_zoom(40, 75); set_bins(1000);
	draw_limits(52.4, 58.5);
	saveaspng(FOLDER + Num+"_SiPMs_trigger_avg_"+cuts_str(cuts));
	set_as_run_cut("SiPMs_trigger"); cuts.push_back(Num);
	post_processor->set_trigger_offsets(55.1);
	Num = int_to_str(++no, 2);
...
set_corr(AStates::PMT_Npe_sum, AStates::PMT_T_sum, -1, -1);
	...
	set_zoom(0, 1000, 0, 15);
	set_bins(500, 250);
	x_y_regions = {341, 5.25, 302, 6.34, 355, 7.54, 435, 7.27};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	set_titles("N_{pe} sPMT t#in[0, 160] #mus", "Signal width trigger, [#mus]");
	saveaspng(FOLDER + Num+"_triggerPMT_Tsum_vs_slowPMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
...
form_n = "forms_Alpha_peak_v1/";
	ty(AStates::Correlation);
	x_y_regions = {385, 5.15, 475, 6.93, 568, 6.98, 575, 5.52};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	set_titles("N_{pe} sPMT t#in[0, 160] #mus", "Signal width trigger, [#mus]");
	saveaspng(FOLDER + Num+"_triggerPMT_Tsum_vs_slowPMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
