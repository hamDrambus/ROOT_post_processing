...
//First analysis of cryogenic temperature gaseous Ar alpha data
...
set_corr(AStates::PMT_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	x_y_regions = {566, 403, 444, 510, 583, 664, 690, 525};
...
form_n = "forms_Alpha_peak_v2/";
	ty(AStates::Correlation);
	x_y_regions = {369, 700, 308, 720, 292, 836, 380, 921, 466, 817, 423, 734};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_slowPMTs_vs_SiPMs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
...
set_corr(AStates::PMT_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	set_bins(1500);
	set_titles("N_{pe} PMTs", "N_{pe} SiPM-matrix");
	x_y_regions = {523, 357, 417, 423, 521, 542, 588, 416};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_slowPMTs_vs_SiPMs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
...
form_n = "forms_Alpha_peak_v2/";
	ty(AStates::Correlation);
	x_y_regions = {354, 530, 316, 579, 318, 671, 386, 707, 448, 637, 425, 557};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_slowPMTs_vs_SiPMs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
