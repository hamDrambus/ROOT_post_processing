...
//Same as 210316/3analysis_history.cpp: double event cut by SiPM [t1,t2]vs[0,160] and sPMT [t1,t2]vs[0,160]
//and track selection signal width by SiPM pulse-shape fit vs sPMT Npe
data_output_path = "210302/results_v4/";
...
set_corr(AStates::MPPC_Npe_sum, AStates::MPPC_shape_fit, -1, -1);
	ty(AStates::Correlation);
	set_zoom(0, 1500, 0, 6);
	set_bins(1500, 500);
	set_titles("N_{pe} SiPMs t#in[0, 160] #mus", "SiPMs fitted width [#mus]");
	update();
	saveaspng(FOLDER + Num + "_SiPM_width_vs_SiPMs_Npe_"+cuts_str(cuts));
	Num = int_to_str(++no, 2);

std::string form_n = "forms_Alpha_peak_v1/"; //v1 - orthogonal tracks, v2 - longitudial
set_corr(AStates::PMT_Npe_sum, AStates::MPPC_shape_fit, -1, -1);
	ty(AStates::Correlation);
	set_zoom(0, 1000, 0, 6);
	set_bins(1000, 500);
	set_titles("N_{pe} sPMT t#in[0, 160] #mus", "SiPMs fitted width [#mus]");
	x_y_regions = {770, 1.70, 447, 1.70, 350, 2.75, 770, 2.75};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_SiPM_width_vs_PMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
...
ty(AStates::Correlation);
	x_y_regions = {516, 3.70, 314, 3.70, 375, 4.82, 469, 4.82};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_SiPM_width_vs_PMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
...
ty(AStates::Correlation);
	x_y_regions = {581, 2.85, 353, 2.85, 322, 3.52, 546, 3.52};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_SiPM_width_vs_PMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
...
set_corr(AStates::PMT_Npe_sum, AStates::MPPC_shape_fit, -1, -1);
	...
	set_bins(1000, 500);
	set_titles("N_{pe} sPMT t#in[0, 160] #mus", "SiPMs fitted width [#mus]");
	x_y_regions = {769, 1.61, 373, 1.61, 322, 2.46, 769, 2.46};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_SiPM_width_vs_PMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
...
form_n = "forms_Alpha_peak_v2/"; //v1 - orthogonal tracks, v2 - longitudial
ty(AStates::Correlation);
	x_y_regions = {460, 3.19, 306, 3.19, 328, 4.01, 415, 4.01};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_SiPM_width_vs_PMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
...
form_n = "forms_Alpha_peak_v3/"; //v3 - middle case
ty(AStates::Correlation);
	x_y_regions = {500, 2.60, 331, 2.60, 311, 3.11, 467, 3.11};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_SiPM_width_vs_PMTs_Npe_"+cuts_str(cuts)+"_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
