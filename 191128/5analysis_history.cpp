...
if (channel==3) {
  cut_A_S_upper(0.013, 0.004, 0.027, 0.0082, display, channel, "rem_smallA_largeS");
...
if (channel==4) {
	cut_A_S_upper(0.013, 0.004, 0.022, 0.0061, display, channel, "rem_smallA_largeS_1");
	cut_A_S_upper(0.013, 0.010, 0.030, 0.012, display, channel, "rem_smallA_largeS_2");
	cut_A_S_lower(0.0165, 0.00082, 0.055, 0.0036, display, channel, "rem_smallA_smallS");
...
if (channel==55) {
	...
	x_y_regions = {0.0094, 0.0094, 0.00053, 0.06, 0.0061, 1e3};
	cut_A_S_fast_PMT(x_y_regions, display, channel, "small_A-S_noise2");
	cut_A_S_upper(0, 0.04, 1, 0.04, display, channel, "rem_S>0.04");
	if (aggressiveness>=1)//select only 1 photoelectron for calibration
...
if (channel==56) {
	...
	x_y_regions = {0.0119, 0.0119, 0.00082, 0.06, 0.0060, 1e3};
	cut_A_S_fast_PMT(x_y_regions, display, channel, "small_A-S_noise2");
	cut_A_S_upper(0, 0.04, 1, 0.04, display, channel, "rem_S>0.04");
	if (aggressiveness>=1)//select only 1 photoelectron for calibration
		cut_A_S_upper(0.0255, 0.0001, 1, 0.0001, display, channel, "2pe");
...
data_output_path = "191128/results_v5/";
calibration_file = "191128/results_v5/191128_calibration.dat";
post_processor->calibr_info.Load(calibration_file);
std::string iter_str = int_to_str((std::size_t)iteration, 2);

std::map<std::string, std::pair<double, double> > S2_times;
S2_times["191128_Pu_20kV_850V_46V_12dB"] = std::pair<double, double> (51, 70);

std::map<std::string, std::string> exp_folders;
exp_folders["191128_Pu_20kV_850V_46V_12dB"] = "Alpha_46V_20kV_850V";
...
if (exp == "191128_Pu_20kV_850V_46V_12dB") {
...
ty(AStates::Correlation);
	x_y_regions = {2, 1, 2, 10, 8, 10, 8, 1};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	set_as_run_cut("En_spec");	cuts = Num;
	saveaspng(FOLDER + Num + "_SiPMs_Npe_vs_4PMT_Npe_"+cuts);
	Num = int_to_str(++no, 2);
	print_accepted_events(FOLDER + "forms_1/events.txt", first_run);
	remcut(-1, "1");

save_forms(FOLDER + "forms_1/", false, PMT_state, SiPM_state);
	unset_as_run_cut("En_spec");

ty(AStates::Correlation);
	x_y_regions = {20, 0, 20, 30, 30, 30, 30, 0};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	set_as_run_cut("En_spec");	cuts = Num;
	saveaspng(FOLDER + Num + "_SiPMs_Npe_vs_4PMT_Npe_"+cuts);
	Num = int_to_str(++no, 2);
	print_accepted_events(FOLDER + "forms_2/events.txt", first_run);
	remcut(-1, "1");

save_forms(FOLDER + "forms_2/", false, PMT_state, SiPM_state);
	unset_as_run_cut("En_spec");

ty(AStates::Correlation);
	x_y_regions = {50, 4, 50, 40, 90, 40, 90, 4};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	set_as_run_cut("En_spec");	cuts = Num;
	saveaspng(FOLDER + Num + "_SiPMs_Npe_vs_4PMT_Npe_"+cuts);
	Num = int_to_str(++no, 2);
	print_accepted_events(FOLDER + "forms_3/events.txt", first_run);
	remcut(-1, "1");

save_forms(FOLDER + "forms_3/", false, PMT_state, SiPM_state);
	unset_as_run_cut("En_spec");

	save_forms(FOLDER + "forms_1/", FOLDER + "forms_1/A_4PMT_fast_by_S", 800, 3);//For next iteration trigger fit
	save_forms(FOLDER + "forms_1/", FOLDER + "forms_1/A_SiPMs_by_Npe", 800, 5);
	save_forms(FOLDER + "forms_1/", FOLDER + "forms_1/A_s4PMT_by_Npe", 800, 8);
	save_forms(FOLDER + "forms_2/", FOLDER + "forms_2/A_4PMT_fast_by_S", 800, 3);
	save_forms(FOLDER + "forms_2/", FOLDER + "forms_2/A_SiPMs_by_Npe", 800, 5);
	save_forms(FOLDER + "forms_2/", FOLDER + "forms_2/A_s4PMT_by_Npe", 800, 8);
	save_forms(FOLDER + "forms_3/", FOLDER + "forms_3/A_4PMT_fast_by_S", 800, 3);
	save_forms(FOLDER + "forms_3/", FOLDER + "forms_3/A_SiPMs_by_Npe", 800, 5);
	save_forms(FOLDER + "forms_3/", FOLDER + "forms_3/A_s4PMT_by_Npe", 800, 8);
}
//END OF FORMS
}
