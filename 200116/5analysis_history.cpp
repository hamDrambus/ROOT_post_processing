...
if (channel==2) {
	...
	cut_A_S_upper(0.024, 0.00146, 0.038, 0.0074, display, channel, "rem_smallA_largeS_2");
...
data_output_path = "200116/results_v5/";
calibration_file = "200116/results_v5/200116_calibration.dat";
post_processor->calibr_info.Load(calibration_file);
std::string iter_str = int_to_str((std::size_t)iteration, 2);

std::map<std::string, std::pair<double, double> > S2_times;
S2_times["200116_Pu_20kV_850V_46V_12dB"] = std::pair<double, double> (21, 40);

std::map<std::string, std::string> exp_folders;
exp_folders["200116_Pu_20kV_850V_46V_12dB"] = "Alpha_46V_20kV_850V";
...
if (exp == "200116_Pu_20kV_850V_46V_12dB") {
...
std::string form_n = "forms_1/";
ty(AStates::Correlation);
	x_y_regions = {3, 2, 3, 10, 8, 10, 8, 2};
...
form_n = "forms_3/";
ty(AStates::Correlation);
	x_y_regions = {11, 2, 11, 11, 18, 11, 18, 2};
	cut_x_y_poly_select(x_y_regions, true, "1");
	update();
	saveaspng(FOLDER + Num + "_SiPMs_Npe_vs_4PMT_Npe_"+cuts);
	set_as_run_cut("En_spec");	cuts = Num;
...
form_n = "forms_4/";
ty(AStates::Correlation);
		x_y_regions = {11, 0, 11, 11, 18, 11, 18, 0};
		cut_x_y_poly_select(x_y_regions, true, "1");
		update();
		saveaspng(FOLDER + Num + "_SiPMs_Npe_vs_4PMT_Npe_"+cuts);
		set_as_run_cut("En_spec");	cuts = Num;
	...
	form_n = "forms_5/";
	ty(AStates::Correlation);
		x_y_regions = {25, 0, 25, 10, 40, 10, 40, 0};
		cut_x_y_poly_select(x_y_regions, true, "1");
		update();
		saveaspng(FOLDER + Num + "_SiPMs_Npe_vs_4PMT_Npe_"+cuts);
		set_as_run_cut("En_spec");	cuts = Num;
