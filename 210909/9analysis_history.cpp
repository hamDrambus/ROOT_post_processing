...
//Modified 8analysis_history.cpp
//Studing coordinate 2D dispersion for SiPM-matrix for alpha events
//Studing these distributions for fast only and slow+long only photoelectons

data_output_path = "210909/results_v9/";
...
set_corr(AStates::MPPC_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	x_y_regions = {44, 0, 70, 13, 96, 21.7, 199, 106};
...
ty(AStates::Correlation_y);
	...
	ty(AStates::Correlation);
	set_zoom(0, 200, 0, 200); set_bins(200);
	x_y_regions = {40, 0, 111, 38.5, 199, 129};
	cut_x_y_right(x_y_regions, true, "1");
	update();
...
ty(AStates::MPPC_Npe_profile);
	...
	saveas(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_"+S2_start+"-160us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(0, d_S2_start); update();
	saveaspng(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	saveas(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_0-"+S2_start+"us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_start, d_S2_finish); update();
	saveaspng(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_"+S2_start+"-"+S2_finish+"us");
	saveas(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_"+S2_start+"-"+S2_finish+"us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_finish, slow_cutoff); update();
	saveaspng(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_"+S2_finish+"-"+s_slow_cutoff+"us");
	saveas(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_"+S2_finish+"-"+s_slow_cutoff+"us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_finish, 160); update();
	saveaspng(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_"+S2_finish+"-160us");
	saveas(FOLDER + Num + "_SiPMs_profile_xy_"+cuts_str(cuts)+"_"+S2_finish+"-160us");
	Num = int_to_str(++no, 2);
