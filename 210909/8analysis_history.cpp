...
//Modified 6analysis_history.cpp
//Studing x-y coordinate distribution for SiPM-matrix with THGEM1 for alpha events
//And Npe profile for these events.
//Studing these distributions for fast only and slow+long only photoelectons
data_output_path = "210909/results_v8/";
...
set_corr(AStates::MPPC_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	set_zoom(0, 3000, 0, 3000); set_bins(1000);
...
ty(AStates::MPPC_Npe_sum);
	...
	set_bins(0, 1000);
	draw_limits(242, 383);
	saveaspng(FOLDER + Num + "_SiPMs_Npe_"+cuts_str(cuts)+"_"+S2_start+"-"+S2_finish+"us_N="+int_to_str(post_processor->numOfFills(true))+"events");
	set_as_run_cut("En_spec"); cuts.push_back(Num);
	print_accepted_events(FOLDER + Num + "_events.txt", first_run);
	unset_draw_limits();
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_finish, 160); set_zoom(0, 1500); set_bins(500);
	saveaspng(FOLDER + Num + "_SiPMs_Npe_"+cuts_str(cuts)+"_"+S2_finish+"-160us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_start, 160); set_zoom(0, 2000); set_bins(500);
	saveaspng(FOLDER + Num + "_SiPMs_Npe_"+cuts_str(cuts)+"_"+S2_start+"-160us");
	Num = int_to_str(++no, 2);

ty(AStates::MPPC_coord);
	cut_SiPM_noise(SiPM_state);
	time_zoom_SiPMs(d_S2_start, 160);
	set_zoom(-12, 12, -12, 12);	set_bins(100);
...
ty(AStates::MPPC_Npe_profile);
	cut_SiPM_noise(SiPM_state);
	time_zoom_SiPMs(d_S2_start, 160);
	set_zoom(-22.5, 22.5, -22.5, 22.5);	set_bins(9);
	saveaspng(FOLDER + Num + "_SiPMs_profile_"+cuts_str(cuts)+"_"+S2_start+"-160us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_start, d_S2_finish); update();
	saveaspng(FOLDER + Num + "_SiPMs_profile_"+cuts_str(cuts)+"_"+S2_start+"-"+S2_finish+"us");
	Num = int_to_str(++no, 2);

	time_zoom_SiPMs(d_S2_finish, 160); update();
	saveaspng(FOLDER + Num + "_SiPMs_profile_"+cuts_str(cuts)+"_"+S2_finish+"-160us");
	Num = int_to_str(++no, 2);
...
set_corr(AStates::MPPC_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	time_zoom_SiPMs(d_S2_start, d_S2_finish);
	ty(AStates::Correlation);
	set_zoom(0, 2000, 0, 2000); set_bins(1000);
...
ty(AStates::MPPC_Npe_sum);
	...
	time_zoom_SiPMs(d_S2_start, 160); set_bins(0, 1200);;
	saveaspng(FOLDER + Num + "_SiPMs_Npe_"+cuts_str(cuts)+"_"+S2_start+"-160us");
	Num = int_to_str(++no, 2);
...
set_corr(AStates::MPPC_Npe_sum, AStates::MPPC_Npe_sum, -1, -1);
	...
	time_zoom_SiPMs(d_S2_start, d_S2_finish);
	ty(AStates::Correlation);
	set_zoom(0, 1200, 0, 1200); set_bins(1200);
...
ty(AStates::MPPC_coord);
	cut_SiPM_noise(SiPM_state);
	time_zoom_SiPMs(d_S2_start, 160);
	set_zoom(-12, 12, -12, 12);	set_bins(80);
	saveaspng(FOLDER + Num + "_SiPMs_xy_"+cuts_str(cuts)+"_"+S2_start+"-160us");
	Num = int_to_str(++no, 2);
...
ty(AStates::MPPC_coord);
	time_zoom_SiPMs(d_S2_start, 160);
	x_y_regions = {-0.90, -1.80, 0.90, -1.80, 1.56, 0, 0.90, 1.80, -0.90, 1.80, -1.56, 0.0};
