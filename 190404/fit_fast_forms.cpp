...
int fit_fast_forms (void) {
    ...
    double time_pretrigger_left = 7, time_pretrigger_right = 25;
	...
	std::vector<double> norm_t_right =  {34.2, 34.3, 34.0, 32.0, 32.5, 33.0, 33.5}; //20, 18, ..., 8 kV
	//std::vector<double> fit_from (7, 27.5);
	//std::vector<double> fit_to =        {30.4, 30.5, 30.9, 31.2, 32.0, 32.2, 32.2}; //SiPM 20, 18, ..., 8 kV
	std::vector<std::string> Tds = {"12kV, 5.1", "14kV, 5.9", "16kV, 6.8"};	
	std::vector<double> fit_from = {29.5, 29.5, 30.2};
	std::vector<double> fit_to = {33.8, 33.8, 33.3};
	std::vector<Color_t> palette_major = {kGreen, kRed, kBlue, kGreen, kRed, kBlue, kYellow + 2, kMagenta, kOrange + 7};
	std::vector<Color_t> palette_minor = {kGreen -2, kMagenta, kAzure + 10, kGray + 2, kGreen -2,  kAzure + 10, kMagenta+3, kOrange - 7, kOrange + 6};
    double max_val = 0;
	bool linear = true;
	...
	std::string framename = std::string("190404 (no WLS) Signal forms (v3) Cd peak fPMTs");// + " " + Tds[0] + " Td";
	...
	for (int hh = 0, hh_end_ = hists.size(); hh!=hh_end_; ++hh) {
		...
		ffs[hh]->SetParLimits(1, 31.5, 32.5);
		ffs[hh]->SetParLimits(2, 0.1, 5);
		ffs[hh]->SetParLimits(3, 0.1, 3);
		ffs[hh]->SetParameters(5, 32, 0.5, 1.3);
	...
	if (!linear) { 
		...
		double ypos1 = 0.17;
		for (int hh = 0, hh_end_ = tau1.size(); hh!=hh_end_; ++hh) {
			auto *txt1 = new TLatex (35, ypos0+0.03*(hh_end_ - hh - 1), (std::string("#Delta=")+tau1[hh]).c_str());
			txt1->SetTextAlign(12); txt1->SetTextSize(0.05);
			txt1->SetTextColor(palette_major[hh]); txt1->Draw();

			auto *txt11 = new TLatex (37, ypos1+0.03*(hh_end_ - hh - 1), (std::string("#sigma=")+tau2[hh]).c_str());
			txt11->SetTextAlign(12); txt11->SetTextSize(0.05);
			txt11->SetTextColor(palette_major[hh]); txt11->Draw();
		}
	}
	
	legend->AddEntry(hist_1, (std::string(Tds[0] + " Td fPMTs Cd peak, Tdrift=3.72#mus")).c_str(), "l");
	legend->AddEntry(hist_2, (std::string(Tds[1] + " Td fPMTs Cd peak, Tdrift=3.22#mus")).c_str(), "l");
	legend->AddEntry(hist_3, (std::string(Tds[2] + " Td fPMTs Cd peak, Tdrift=2.80#mus")).c_str(), "l");
